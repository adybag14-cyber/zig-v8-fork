...
fn addDepotToolsToPath(step: *std.Build.Step.Run, depot_tools_dir: []const u8) void {
    ...
    if (builtin.os.tag == .windows) {
        // The staged depot_tools snapshot is immutable and this build uses the
        // Visual Studio/Windows SDK already installed on the host.
        step.setEnvironmentVariable("DEPOT_TOOLS_UPDATE", "0");
        step.setEnvironmentVariable("DEPOT_TOOLS_WIN_TOOLCHAIN", "0");
    }
}
...
fn bootstrapDepotTools(b: *std.Build, depot_tools_dir: []const u8) !*std.Build.Step {
    ...
    const write_telemetry_config = if (builtin.os.tag == .windows) blk: {
        const cmd = b.addSystemCommand(&.{
            "python",
            "-c",
            "import pathlib, sys; pathlib.Path(sys.argv[1]).write_text(sys.argv[2], encoding='utf-8')",
        });
        cmd.addArg(b.fmt("{s}/build_telemetry.cfg", .{depot_tools_dir}));
        cmd.addArg(build_telemetry_config_content);
        break :blk cmd;
    } else blk: {
        const cmd = b.addSystemCommand(&.{ "sh", "-c" });
        cmd.addArg(b.fmt("echo '{s}' > {s}/build_telemetry.cfg", .{
    ...
    if (builtin.os.tag == .windows) {
        const windows_bootstrap = b.addSystemCommand(&.{
            "cmd.exe",                  "/d", "/s", "/c",
            "bootstrap\\win_tools.bat",
        });
        windows_bootstrap.setCwd(.{ .cwd_relative = depot_tools_dir });
        addDepotToolsToPath(windows_bootstrap, depot_tools_dir);
        windows_bootstrap.step.dependOn(&write_telemetry_config.step);
        ensure_bootstrap.step.dependOn(&windows_bootstrap.step);
    } else {
...
fn bootstrapV8(
    ...
    const write_gclient = if (builtin.os.tag == .windows) blk: {
        const cmd = b.addSystemCommand(&.{
            "python",
            "-c",
            "import pathlib, sys; pathlib.Path(sys.argv[1]).write_text(sys.argv[2], encoding='utf-8')",
        });
        cmd.addArg(b.fmt("{s}/.gclient", .{v8_dir}));
        cmd.addArg(gclient_content);
        break :blk cmd;
    } else blk: {
        const cmd = b.addSystemCommand(&.{ "sh", "-c" });
        cmd.addArg(b.fmt("echo '{s}' > {s}/.gclient", .{ gclient_content, v8_dir }));
        break :blk cmd;
    };
    ...
    const write_gclient_args = if (builtin.os.tag == .windows) blk: {
        const cmd = b.addSystemCommand(&.{
            "python",
            "-c",
            "import pathlib, sys; pathlib.Path(sys.argv[1]).write_text(sys.argv[2], encoding='utf-8')",
        });
        cmd.addArg(b.fmt("{s}/build/config/gclient_args.gni", .{v8_dir}));
        cmd.addArg("# Generated by Zig build system\n");
        break :blk cmd;
    } else blk: {
        const cmd = b.addSystemCommand(&.{ "sh", "-c" });
        cmd.addArg(b.fmt("echo '# Generated by Zig build system' > {s}/build/config/gclient_args.gni", .{v8_dir}));
        break :blk cmd;
    };
    write_gclient_args.step.dependOn(&mkdir_build_config.step);

    // Run gclient sync
    const gclient_sync = if (builtin.os.tag == .windows)
        b.addSystemCommand(&.{
            "cmd.exe",                                              "/d",   "/s", "/c",
            getDepotToolExePath(b, depot_tools_dir, "gclient.bat"), "sync",
        })
    else
        b.addSystemCommand(&.{
    ...
    // Run clang update
    const clang_update = if (builtin.os.tag == .windows)
        b.addSystemCommand(&.{
            "cmd.exe",                                                         "/d",                            "/s", "/c",
            getDepotToolExePath(b, depot_tools_dir, "python-bin/python3.bat"), "tools/clang/scripts/update.py",
        })
    else
        b.addSystemCommand(&.{
...
fn buildV8(
    ...
    if (needs_build) {
        const gn_run = if (builtin.os.tag == .windows)
            b.addSystemCommand(&.{
                "cmd.exe",                                         "/d",       "/s",                                "/c",
                getDepotToolExePath(b, depot_tools_dir, "gn.bat"), "--root=.", "--root-target=//zig",               "--dotfile=zig/.gn",
                "gen",                                             out_dir,    b.fmt("--args={s}", .{args_string}),
            })
        else
            b.addSystemCommand(&.{
        ...
        const ninja_run = if (builtin.os.tag == .windows)
            b.addSystemCommand(&.{
                "cmd.exe",                                                "/d", "/s",    "/c",
                getDepotToolExePath(b, depot_tools_dir, "autoninja.bat"), "-C", out_dir, "c_v8",
            })
        else
            b.addSystemCommand(&.{
